...
echo_listener(void *data) {
	...
	server_socket = reliable_connection(NET_PASSIVE, 0, true,
			"/tmp/tcpipiv3.serv"); //tcp_server(8080);
...
int main(int argc, char **args) {
	...
	bool init_result = init_producer_consumer(&prod_cons, 1, 5, sizeof(int),
			100, echo_listener, echo_handler, NULL, NULL);
	if (init_result) {
		printf("INIT SUCCESS\n");
