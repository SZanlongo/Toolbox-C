...
void produce(producer_consumer *prod_cons, void *obj) {
	pthread_mutex_lock(prod_cons->cond_mutex);
	produce_loop: pthread_mutex_lock(prod_cons->exclusion_mutex);
	...
	if (buffer_items < prod_cons->buffer_max) {
		if (prod_cons->buffer_head > 0) {
			int counter = 0;
			//shift
			for (int i = prod_cons->buffer_head; i < prod_cons->buffer_tail;
					++i) {
...
internal_consume(void *pc) {
	...
	while (1) {
		pthread_mutex_lock(prod_cons->cond_mutex);
		consume_loop: pthread_mutex_lock(prod_cons->exclusion_mutex);
		...
		if (buffer_items > 0) {
			...
			memcpy(item, *(prod_cons->buffer + prod_cons->buffer_head),
					prod_cons->buffer_item_size);
...
bool init_producer_consumer(producer_consumer *prod_cons, uint32_t producers,
		uint32_t consumers, size_t size_of_obj, uint32_t max_queue,
		void *(*produce_function)(void *), void *(*consume_function)(void *),
		void *(consume_startup)(void *), void *consume_startup_args) {
	...
	if (!init_pool(prod_cons->producer_pool, producers, produce_function, NULL)
			|| !init_pool(prod_cons->consumer_pool, consumers, internal_consume,
					prod_cons)) {
	...
	prod_cons->exclusion_mutex = (pthread_mutex_t *) malloc(
